"""Broader controlled baseline production model smoke test.

This Stage 5.9B script executes the seven BaselineProduction models for the
first three eligible entities and one most-recent window per entity. It does
not modify execution.yaml, run challengers, calculate metrics, create rankings,
or publish tournament outputs.
"""
...
logger = get_logger("run_baseline_broader_smoke_test")
...
SMOKE_FORECASTS_OUTPUT = SMOKE_DIR / "broader_smoke_forecasts.csv"
SMOKE_STATUS_OUTPUT = SMOKE_DIR / "broader_smoke_execution_status.csv"
SMOKE_SUMMARY_OUTPUT = SMOKE_DIR / "broader_smoke_summary.csv"
...
def _require_file(path) -> None:
    """Validate that a required broader smoke-test input exists."""

    if not path.exists():
        raise FileNotFoundError(f"Required broader smoke-test input missing: {path}")
...
def _select_entities() -> list[str]:
    """Select the first three eligible entities from the window summary."""

    _require_file(BACKTESTING_WINDOW_SUMMARY)
    summary = pd.read_csv(BACKTESTING_WINDOW_SUMMARY)
    entities = sorted(summary["entity_key"].dropna().unique())[:3]
    if len(entities) != 3:
        raise ValueError(f"Expected 3 eligible entities, found {len(entities)}.")
    logger.info("Selected broader smoke entities: %s", entities)
    return entities
...
def _select_smoke_jobs(baseline_jobs: pd.DataFrame) -> pd.DataFrame:
    ...
    smoke_jobs = pd.concat(selected_rows, ignore_index=True)
    if len(smoke_jobs) != 21:
        raise ValueError(f"Expected 21 broader smoke jobs, found {len(smoke_jobs)}.")
    return smoke_jobs
...
def _forecast_dates(job: pd.Series) -> pd.DatetimeIndex:
    ...
    if len(dates) != 30:
        raise ValueError(f"Broader smoke test requires 30 dates for {job['job_id']}.")
    return dates
...
def _write_outputs(
    ...
    """Write all broader smoke-test outputs."""
...
def run_baseline_broader_smoke_test() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Execute the controlled 21-job broader baseline smoke test."""

    logger.info("Stage 5.9B broader baseline smoke test started")
    run_id = f"broader_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for _, job in smoke_jobs.iterrows():
        try:
            ...
            status = "completed"
            message = "Broader smoke test job completed."
    ...
    summary = pd.DataFrame(
        [
            {
                "run_id": run_id,
                "entities_tested": smoke_jobs["entity_key"].nunique(),
                "windows_tested": smoke_jobs[["entity_key", "window_id"]]
                .drop_duplicates()
                .shape[0],
                "models_tested": smoke_jobs["model_name"].nunique(),
    ...
    logger.info("Stage 5.9B broader baseline smoke test completed")
    return forecasts, statuses, summary


if __name__ == "__main__":
    run_baseline_broader_smoke_test()
